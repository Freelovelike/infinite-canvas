...
+ [调整] OpenAI 风格视频接口的图片与视频参考合计上限调整为 9 个。
+ [新增] OpenAI 风格视频接口支持将画布视频节点作为参考视频上传，可直接对接 Migoo 视频续写。
+ [新增] 画布支持安装3D导演台插件，可通过场景描述和参考图生成全景背景、进入完整导演台布置角色与机位，并将拍摄结果回传为图片节点。
+ [优化] 视频生成提示词直接展开文本引用并仅以附件传递媒体引用，不再附加文本和图片节点编号清单。
+ [修复] 画布持久化视频任务 ID 并在刷新后恢复轮询，临时网络错误不再导致已提交任务与结果失联。
+ [修复] OpenAI 风格视频任务完成后携带 API Key 下载受保护的成片并写入画布，避免后台可播放但画布节点因内容请求 401 而空白。
