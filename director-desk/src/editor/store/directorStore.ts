...
let directorScenePersistenceEnabled = true;

export function setDirectorScenePersistenceEnabled(enabled: boolean) {
  directorScenePersistenceEnabled = enabled;
}
...
function writePersistedDirectorState(state: DirectorState) {
  if (!directorScenePersistenceEnabled) return;
  const storage = getLocalStorageSafe();
  if (!storage) return;
  ...
  return {
    ...
    openScopedScene: (scopeId) => {
      ...
      const snapshot = createInitialDirectorState({
        ...
        includePersistedScene: directorScenePersistenceEnabled,
